...
export default function JournalsPage() {
  return (
    <main className="min-h-screen bg-[#FCFAF6]">
      ...
      <section className="mx-auto max-w-[1100px] px-6 py-10 md:py-16">{journalPosts.map((post, index) => <article key={post.slug} className="grid gap-7 border-b border-black/10 py-9 first:pt-0 md:grid-cols-[.9fr_1.1fr] md:items-center md:gap-14"><Link href={`/journals/${post.slug}`} className={`group relative block aspect-[4/3] overflow-hidden bg-[#f0e5d3] ${index % 2 ? "md:order-2" : ""}`}><Image src={post.image} alt={post.title} fill className="object-contain p-8 transition-transform duration-700 group-hover:scale-[1.04]" sizes="(min-width: 768px) 45vw, 100vw" /></Link><div className={index % 2 ? "md:order-1" : ""}><p className="text-[9px] tracking-[.18em] text-black/55">{post.tag.toUpperCase()}</p><Link href={`/journals/${post.slug}`} className="mt-3 block font-serif text-3xl leading-[.9] tracking-[-.02em] transition-opacity hover:opacity-55 md:text-4xl">{post.title}</Link><p className="mt-4 max-w-md text-sm leading-relaxed text-black/60">{post.excerpt}</p><Link href={`/journals/${post.slug}`} className="u-line mt-6 inline-block pb-0.5 text-[10px] tracking-[.14em]">READ THE NOTE</Link></div></article>)}</section>
