    ...
    <main className="min-h-screen bg-[#FCFAF6]">
      ...
      <section className="mx-auto max-w-[1280px] px-5 py-10 sm:px-6 md:px-8 md:py-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
          {products.map((product) => <article key={product.id} className="group relative flex min-h-[390px] flex-col border-b border-black/10 px-3 py-5 transition-colors duration-500 hover:bg-[#f0eadf]/50 sm:px-5 md:min-h-[500px] md:border-r md:last:border-r-0">
            <Link href={`/shop/${product.id}`} className="absolute inset-0 z-10" aria-label={`View ${product.name} — ${product.flavour}`} />
            <div className="relative z-0 flex flex-1 items-center justify-center overflow-hidden"><Image src={product.src} alt={`${product.name} — ${product.flavour}`} width={280} height={390} className="can-shadow h-[69%] w-auto object-contain transition-transform duration-700 group-hover:scale-[1.03]" /></div>
            <div className="relative z-20 pt-4"><div className="flex justify-between gap-2"><h2 className="flex items-center gap-1.5 text-[11px] font-medium leading-tight"><span className="h-1.5 w-1.5 shrink-0 rounded-full transition-transform duration-300 group-hover:scale-125" style={{ backgroundColor: product.shade }} />{product.name}</h2><span className="shrink-0 text-[10px]">£{product.price}</span></div><p className="mt-1 text-[9px] font-medium text-black/70">{product.flavour}</p><p className="mt-1 min-h-[36px] text-[9px] leading-relaxed text-black/55 line-clamp-2">{product.description}</p><p className="mt-1 text-[8px] tracking-wide text-black/45">{product.content}</p><button onClick={() => handleAdd(product.id)} className={`u-line relative z-30 mt-4 self-start pb-0.5 text-[9px] tracking-[.15em] ${added === product.id ? "font-medium" : ""}`}>{added === product.id ? "ADDED ✓" : "ADD TO CART"}</button></div>
          </article>)}
        </div>
